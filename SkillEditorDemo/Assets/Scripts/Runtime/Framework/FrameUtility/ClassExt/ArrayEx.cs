...
namespace Module.Utility
{
    ...
    {
        public static string Join<T>(this Array array, char seq)
        {
            var ret = "";
            for (var i = 0; i < array.Length; i++)
            {
                ret = ret + (T) array.GetValue(i);
                var r = i < array.Length - 1 ? seq.ToString() : "";
                ret = ret + r;
            }
        ...
        {
            for (var i = 0; i < array.Length; i++) act(i, (T) array.GetValue(i));
        }
        ...
        {
            for (var i = 0; i < array.Length; i++) act((T) array.GetValue(i));
        }
        ...
        {
            var res = new T[array.Length + anotherArray.Length];
        ...
        public static T[] CutHeadByLength<T>(ref T[] array, int length)
        {
            if (length > array.Length)
                length = array.Length;
            var head = new T[length];
            var tail = new T[array.Length - length];
