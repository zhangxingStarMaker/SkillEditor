...
namespace Module.FrameBase
{
    ...
    public class IdGenerator: Singleton<IdGenerator>
    {
        ...
        public IdGenerator()
        {
            this.epochThisYear = new DateTime(DateTime.Now.Year, 1, 1, 0, 0, 0, DateTimeKind.Utc).Ticks / 10000000 ;
            
            this.lastInstanceIdTime = TimeSinceThisYear();
            if (this.lastInstanceIdTime <= 0)
            {
                FrameworkLog.LogWarning($"lastInstanceIdTime less than 0: {this.lastInstanceIdTime}");
                this.lastInstanceIdTime = 1;
            }
            this.lastIdTime = this.lastInstanceIdTime;
            if (this.lastIdTime <= 0)
            {
                FrameworkLog.LogWarning($"lastIdTime less than 0: {this.lastIdTime}");
                this.lastIdTime = 1;
            }
        }
        ...
        {
            uint a = (uint)((DateTime.Now.Ticks/10000000 - this.epochThisYear));
            return a;
        }
        ...
        public long GenerateInstanceId()
        {
            ...
            else
            {
                ...
                if (this.instanceIdValue > IdGenerator.Mask18bit - 1) // 18bit
                {
                    ...
                    FrameworkLog.LogError($"instanceid count per sec overflow: {time} {this.lastInstanceIdTime}");
                }
            }
        ...
        public long GenerateId()
        {
            ...
            else
            {
                ...
                if (value > ushort.MaxValue - 1)
                {
                    ...
                    FrameworkLog.LogError($"id count per sec overflow: {time} {this.lastIdTime}");
                }
            }
